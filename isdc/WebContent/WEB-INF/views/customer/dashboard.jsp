	  ...
	  <div class="panel-body"> 
	  	...
	  	<core:if test="${param.message=='create-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Create customer entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='update-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Update customer entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Delete customer entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-fail'}">												
		  	<div class="alert alert-danger">
		        <strong>Error!</strong> Delete customer entry request could not be completed, the customer is assigned to a customer balance entry.
		   	</div>					
		</core:if>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="dashboard" width="100%">
			...
			<tbody>
			<core:forEach items="${customermasterlist}" var="customermasterlist">
				<tr>
					<td><core:out value="${customermasterlist.customer_id}" /></td>
					<td><core:out value="${customermasterlist.customer_name}" /></td>
					<td><fmt:formatDate value="${customermasterlist.customer_created_date}" pattern="dd-MM-yyyy"/></td>
					<td><fmt:formatDate value="${customermasterlist.customer_updated_date}" pattern="dd-MM-yyyy" /></td>
					<spring:url value="/customer/edit?id=${customermasterlist.customer_id}" var="editURL" />
					<spring:url value="/customer/delete?id=${customermasterlist.customer_id}" var="deleteURL" />
					<td>
					...
					<td>
						<spring:url value="/customer/balance/dashboard?customerid=${customermasterlist.customer_id}" var="addBalanceURL" />
						<a href="${addBalanceURL}">Manage</a>
					</td>
				</tr>
			</core:forEach>
			</tbody>
