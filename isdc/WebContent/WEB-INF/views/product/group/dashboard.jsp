...
<title>e-Distribution and Channel Management | Group Dashboard</title>
...
<div class="container">
	<div class="panel panel-default">
	  <div class="panel-heading">
	    <div class="floatleft"><h3 class="panel-title">Group Dashboard</h3></div>
	    <spring:url value="/product/group/add" var="addURL" />
	  ...
	  <div class="panel-body"> 
	  	...
	  	<core:if test="${param.message=='create-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Create group entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='update-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Update group entry request was completed successfully.
		   	</div>					
		</core:if>
	  	<core:if test="${param.message=='delete-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Delete group entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-fail'}">												
		  	<div class="alert alert-danger">
		        <strong>Error!</strong> Delete group entry request could not be completed, the group is assigned to a product.
		   	</div>					
		</core:if>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="dashboard" width="100%">
			...
			<tbody>
			<core:forEach items="${productgrouplist}" var="productgrouplist">
				<tr>
					<td><core:out value="${productgrouplist.product_group_id}" /></td>
					<td><core:out value="${productgrouplist.product_group_name}" /></td>
					<td><core:out value="${productgrouplist.product_group_description}" /></td>
					<td><core:out value="${productgrouplist.product_group_target_amount}" /></td>
					<td><fmt:formatDate value="${productgrouplist.product_group_created_date}" pattern="dd-MM-yyyy"/></td>
					<td><fmt:formatDate value="${productgrouplist.product_group_updated_date}" pattern="dd-MM-yyyy"/></td>
					<spring:url value="/product/group/edit?id=${productgrouplist.product_group_id}" var="editURL" />
					<spring:url value="/product/group/delete?id=${productgrouplist.product_group_id}" var="deleteURL" />
					<td>
