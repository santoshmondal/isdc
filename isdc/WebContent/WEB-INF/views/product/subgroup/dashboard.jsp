...
<title>e-Distribution and Channel Management | Sub Group Dashboard</title>
...
<div class="container">
	<div class="panel panel-default">
	  <div class="panel-heading">
	    <div class="floatleft"><h3 class="panel-title">Sub Group Dashboard</h3></div>
	    <spring:url value="/product/subgroup/add" var="addURL" />
	  ...
	  <div class="panel-body"> 
	  	...
	  	<core:if test="${param.message=='create-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Create sub-group entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='update-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Update sub-group entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Delete sub-group entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-fail'}">												
		  	<div class="alert alert-danger">
		        <strong>Error!</strong> Delete sub-group entry request could not be completed, the sub-group is assigned to a product.
		   	</div>					
		</core:if>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="dashboard" width="100%">
			...
			<tbody>
			<core:forEach items="${productsubgrouplist}" var="productsubgrouplist">
				<tr>
					<td><core:out value="${productsubgrouplist.product_sub_group_id}" /></td>
					<td><core:out value="${productsubgrouplist.product_sub_group_name}" /></td>
					<td><core:out value="${productsubgrouplist.product_sub_group_description}" /></td>
					<td><fmt:formatDate value="${productsubgrouplist.product_sub_group_created_date}" pattern="dd-MM-yyyy"/></td>
					<td><fmt:formatDate value="${productsubgrouplist.product_sub_group_updated_date}" pattern="dd-MM-yyyy"/></td>
					<spring:url value="/product/subgroup/edit?id=${productsubgrouplist.product_sub_group_id}" var="editURL" />
					<spring:url value="/product/subgroup/delete?id=${productsubgrouplist.product_sub_group_id}" var="deleteURL" />
					<td>
