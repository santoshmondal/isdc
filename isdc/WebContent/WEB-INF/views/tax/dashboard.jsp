...
<title>e-Distribution and Channel Management | Tax Dashboard</title>
...
<div class="container">
	<div class="panel panel-default">
	  <div class="panel-heading">
	    <div class="floatleft"><h3 class="panel-title">Tax Dashboard</h3></div>
	    <spring:url value="/tax/add" var="addURL" />
	  ...
	  <div class="panel-body"> 
	  	...
	  	<core:if test="${param.message=='create-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Create tax entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='update-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Update tax entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Delete tax entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-fail'}">												
		  	<div class="alert alert-danger">
		        <strong>Error!</strong> Delete tax entry request could not be completed, the tax is assigned to a product.
		   	</div>					
		</core:if>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="dashboard" width="100%">
			...
			<tbody>
			<core:forEach items="${taxmasterlist}" var="taxmasterlist">
				<tr>
					<td><core:out value="${taxmasterlist.tax_id}" /></td>
					<td><core:out value="${taxmasterlist.tax_name}" /></td>
					<td><core:out value="${taxmasterlist.tax_percentage}" /></td>
					<td><core:out value="${taxmasterlist.tax_description}" /></td>
					<td><core:out value="${taxmasterlist.tax_status}" /></td>
					<td><fmt:formatDate value="${taxmasterlist.tax_created_date}" pattern="dd-MM-yyyy"/></td>
					<td><fmt:formatDate value="${taxmasterlist.tax_updated_date}" pattern="dd-MM-yyyy"/></td>
					<spring:url value="/tax/edit?id=${taxmasterlist.tax_id}" var="editURL" />
					<spring:url value="/tax/delete?id=${taxmasterlist.tax_id}" var="deleteURL" />
					<td>
