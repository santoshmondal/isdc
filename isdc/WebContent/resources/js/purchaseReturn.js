...
PurchaseReturn.prototype.addProduct = function( product ){
	this.productList.push(product);
};

PurchaseReturn.prototype.removeProduct = function(productCode){
...
PurchaseReturn.prototype.getProduct = function(productCode){
	for(var i=0; i < this.productList.length; i++ )
	{
...
$(document).ready( function(){
	...
	$("#invoice_no").blur( function(e){
		...
		if( invoiceNo == purchaseReturn.invoiceNumber ){
			return false;
		}
		
		purchaseReturn = new PurchaseReturn();
		purchaseReturn.invoiceNumber = -1;
		$("#Error").html("");
		...
		$.ajax({
			...
			success:function(result){
				...
				if( object.found == "YES" ){
					...
					purchaseReturn.invoiceNumber = invoiceNo;
	...
	$("#mainTable").on("keydown", ".productCode", function(event){
		...
		if( keyCode == 13){
			...
			if( purchaseReturn.invoiceNumber == -1 ){
	...
	$("#mainTable").on("keydown", ".defected", function(event){
		var keyCode = (event.keyCode) ? event.keyCode : event.which;
	...
	$("#mainTable").on("keyup", ".defected", function(event){
		...
		var PR = purchaseReturn.getProduct(pCode);
	...
	function displayGrandTotal(){
		var grandAmount = 0;
		var grandQuantity = 0;
		var productList = purchaseReturn.productList;
		for(var i=0; i < productList.length; i++ ){
			var pr = productList[i];
			grandAmount = grandAmount + pr.returnFinalPrice;
			grandQuantity = grandQuantity + pr.returnTotalQuantity;
		}
		purchaseReturn.grandAmount = grandAmount;
		purchaseReturn.grandQuantity = grandQuantity;
		$("#grandAmount").html( purchaseReturn.grandAmount );
		$("#grandQuantity").html( purchaseReturn.grandQuantity );
	}
	...
	function calculateAndDisplayTaxs(taxList, RPObject){
			...
			//$("#ROW-"+RPObject.productCode+" .productTaxs").html(taxHtml);
	...
	function saveReturn(){
		...
		if(purchaseReturn.productList.length == 0){
			$("#Error").html( "No product added to return !" );
			return false;
		}
		
		var error = '';
		for(var i=0; i < purchaseReturn.productList.length; i++ ){
			var pr = purchaseReturn.productList[i];
		...
		console.log(JSON.stringify(purchaseReturn));
		$.ajax({
			url:"savePurchaseReturn",
