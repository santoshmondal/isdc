...
<title>e-Distribution and Channel Management | Beat Dashboard</title>
...
<div class="container">
	<div class="panel panel-default">
	  <div class="panel-heading">
	    <div class="floatleft"><h3 class="panel-title">Beat Dashboard</h3></div>
	    <spring:url value="/beat/add?areaid=${param.areaid}" var="addURL" />
	  ...
	  <div class="panel-body"> 
	  	...
	  	<core:if test="${param.message=='create-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Create beat entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='update-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Update beat entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-success'}">												
		  	<div class="alert alert-success">
		        <strong>Success!</strong> Delete beat entry request was completed successfully.
		   	</div>					
		</core:if>
		<core:if test="${param.message=='delete-fail'}">												
		  	<div class="alert alert-danger">
		        <strong>Error!</strong> Delete beat entry request could not be completed, the beat is assigned to a product.
		   	</div>					
		</core:if>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="dashboard" width="100%">
			...
			<tbody>
			<core:forEach items="${beatmasterlist}" var="beatmasterlist">
				<tr>
					<td><core:out value="${beatmasterlist.beat_id}" /></td>
					<td><core:out value="${beatmasterlist.beat_name}" /></td>
					<td><fmt:formatDate value="${beatmasterlist.beat_created_date}" pattern="dd-MM-yyyy"/></td>
					<td><fmt:formatDate value="${beatmasterlist.beat_updated_date}" pattern="dd-MM-yyyy"/></td>
					<spring:url value="/beat/edit?id=${beatmasterlist.beat_id}&areaid=${param.areaid}" var="editURL" />
					<spring:url value="/beat/delete?id=${beatmasterlist.beat_id}&areaid=${param.areaid}" var="deleteURL" />
					<td>
